...
object MapUtil {
    ...
    private fun getBitmapDescriptor(context: Context, id: Int): BitmapDescriptor? {
        ...
        val h = context.resources.getDimension(R.dimen._35sdp).toInt()
        val w = context.resources.getDimension(R.dimen._35sdp).toInt()
    ...
    fun getGeoCoderAddress(context: Context?, location: LatLng?): List<Address>? {
        if (location != null) {
            val geocoder = context?.let { Geocoder(it, Locale.forLanguageTag("ar")) }
            try {
                /*
